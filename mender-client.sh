...
function check_input() {
    [[ -z "$MENDER_SERVER_URL" ]] && { log "WARN: MENDER_SERVER_URL is not set, using default '$MENDER_SERVER_URL'"; }
    [[ -z "$KEYS_DIR" ]] && { log "WARN: KEYS_DIR is not set, using default '$KEYS_DIR'"; }
    [[ -z "${MENDER_TENANT_TOKEN}" ]] && { log "ERROR: missing tenant token"; show_help; exit 1; }
...
function send_deployment_log() {
  # Upload the device deployment log
  [[ -z "$1" ]] && { log "WARNING: unable to send deployment log, missing deploument id" ; return; }
  log "Sending deployment log for the deployment '${1}'"
  curl ${CURL_OPTIONS} -k \
    -H "Authorization: Bearer $JWT" \
    -H "Content-Type: application/json" \
    -d "{\"messages\": [{\"timestamp\": \"2016-03-11T13:03:17.063493443Z\", \"level\": \"INFO\", \"message\": \"OK\"}]}" \
    -X PUT \
    "${MENDER_SERVER_URL}/api/devices/v1/deployments/device/deployments/${1}/log"
}
...
while [ -f "$RUNNING_FILE" ]; do
  ...
  [[ -z "$deployment_json" ]] && { log "ERROR: failed to get deployment"; exit 1; }
  deployment_id=$(jq -r '.id' <<< ${deployment_json})
  deployment_url=$(jq -r '.artifact.source.uri' <<< ${deployment_json})
  [[ -z "$deployment_id" ]] && { log "ERROR: \$deployment_id is empty"; exit 1; }
  [[ -z "$deployment_url" ]] && { log "ERROR: \$deployment_url is empty"; exit 1; }
  ...
  # Push deployment log
  send_deployment_log "${deployment_id}"
done
